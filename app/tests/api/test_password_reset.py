...
async def test_password_reset_full_flow(client, db_session, user_factory):
    ...
    resp1 = await client.post("/api/v1/accounts/password-reset/request/", json={"email": user.email})
    ...
    resp2 = await client.post("/api/v1/accounts/reset-password/complete/", json={
        "email": user.email,
...
async def test_password_reset_invalid_token(client, db_session, user_factory):
    ...
    await client.post("/api/v1/accounts/password-reset/request/", json={"email": user.email})

    response = await client.post("/api/v1/accounts/reset-password/complete/", json={
        "email": user.email,
